...
import { BALANCE, xpToNext, enemyHpMul, enemySpdMul, spawnInterval, bossHp, comboMult } from '../src/config.js';
...
test('bolt stats match the v1 formulas', () => {
  const lv1 = boltStats(1);
  assert.ok(Math.abs(lv1.interval - 0.79) < 1e-9);
  assert.equal(lv1.dmg, 14);
